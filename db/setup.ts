...
export async function ensureDatabase() {
  ...
  await db.batch([
    db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email ON users(email)'), db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_sessions_token ON sessions(token_hash)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_sessions_user ON sessions(user_id)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_nutrition_user_date ON nutrition_entries(user_id, eaten_on)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_nutrition_visibility_date ON nutrition_entries(visibility, eaten_on)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_tasks_user_visibility_status ON tasks(user_id, visibility, status)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_expenses_user_visibility_date ON expenses(user_id, visibility, spent_on)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_organisers_user_visibility ON organiser_items(user_id, visibility)'), db.prepare('CREATE INDEX IF NOT EXISTS idx_messages_created ON messages(created_at)'), db.prepare('INSERT OR IGNORE INTO household_settings (id,name,address,updated_at) VALUES (1,\'Our home\',\'\',?)').bind(new Date().toISOString()), db.prepare('DELETE FROM sessions WHERE expires_at <= ?').bind(new Date().toISOString()),
  ]);
