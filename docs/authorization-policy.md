...
| Medications and dose history            | Private/shareable                   | Shared medication details, supply state, and today's taken state; no historical dose archive | Any member for themselves            | Medication creator only, including dose undo and deletion            |
...
| Live-update cursors                     | Derived personal/household metadata | Only scopes caused by records the member is authorized to read                               | Server after a successful mutation   | Server-managed; clients cannot write cursors                         |
...
- `/api/data` section responses plus `/api/spending` and `/api/history` pages
  apply the same row-level user/visibility predicates as the compatibility
  aggregate. Section selection cannot broaden access, and cursors contain only
