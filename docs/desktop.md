...
The app checks a small privacy-scoped update cursor every five seconds while active instead of downloading the complete household dataset on a timer. Ordinary changes reload only their authorized `/api/data` sections, while chat uses its smaller snapshot. A full catch-up is reserved for first load, reconnect/visibility recovery, or a pruned cursor. Hidden browser tabs back off to 30 seconds. Expense history is bounded to 100 rows per stable cursor page so long-lived households do not inflate every refresh.
